...
Version 1.12.2 is here with the following

App Improvements:
- Notes for source users have been improved.
- Grammatical bugs with the opening splash screen have been fixed.
- Directory keybinds have been improved.
- Missing "again?" prompts in the Password Generator have been added.
""")
...
try:
    ...
    if startup_behaviour == "skip_splash":
        ...
        date_line = f"It's {format_clock_time(now)}, on {now.strftime('%B')} {now.day}, {now.year}."
        ...
        if not PSUTIL_AVAILABLE:
            print(f"\n{RED}Hi there source users! The System Monitor is unavailable - it requires the {CYAN}psutil{RED} module to be installed.{RESET}")
            print(f"To enable it, run {CYAN}pip install psutil{RESET} in your terminal. If that doesn't work, try {CYAN}py -m pip install psutil{RESET}.\n")
            print(f"{YELLOW}Don't worry, the rest of the app's features are fully functional without it!{RESET}\n")
    ...
    while True:
        ...
        while True:
            ...
            choice = input("\n> ").strip().lower()
            if choice in ("q", "quit") or isinstance(KeyboardInterrupt, KeyboardInterrupt):
                print(f"{GREEN}Exiting the app. Thanks for using Termyx!{RESET}")
                time.sleep(0.5) 
                sys.exit()
            elif choice in ("system monitor", "sys") and not PSUTIL_AVAILABLE:
                print(f"{RED}The System Monitor is unavailable — run 'pip install psutil' or 'py -m pip install psutil' in your terminal to enable it.{RESET}")
