...
To run Termyx, users will need the following:

- **Python 3.x (ideally 3.8+)** – Termyx is built in Python, so make sure you have Python 3 installed. You can download it from [python.org](https://www.python.org/downloads/).
- **A PC running Windows 10+** - The system stats uses a module which may not run on macOS or Linux. If this can be tested, that would be greatly appreciated.
- **A terminal or command-line interface** – This could be Command Prompt, PowerShell, or any CLI that can run Python scripts.  
